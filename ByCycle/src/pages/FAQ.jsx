export function FAQ() {
  return (
    <div className="container-fluid flex-row tracking-widest p-10">
      <div className="overflow-wrap: break-word">
        <ol>
          <li>
            <h6 className="font-semibold">1. What is ByCycle all about?</h6>

            <p>
              ByCycle is a community page dedicated to all things related to
              biking, trails, cycling events, and more!
            </p>
          </li>
          <br></br>
          <li>
            <h6 className="font-semibold">
              2. How can I join the ByCycle community?
            </h6>

            <p>
              Simply follow our page and start engaging with other bike
              enthusiasts through posts, quizzes, and discussions.
            </p>
          </li>
          <br></br>
          <li>
            <h6 className="font-semibold">
              3. What kind of trails do you feature on ByCycle?
            </h6>

            <p>
              We showcase a variety of trails for all skill levels, from
              beginner-friendly paths to challenging mountain bike routes.
            </p>
          </li>
          <br></br>
          <li>
            <h6 className="font-semibold">
              4. Are there any upcoming cycling events I should know about?{" "}
            </h6>

            <p>
              Stay tuned to ByCycle for updates on new events, group rides, and
              races in your area.
            </p>
          </li>
          <br></br>
          <li>
            <h6 className="font-semibold">
              5. Can I participate in cycling quizzes on ByCycle?{" "}
            </h6>

            <p>
              {" "}
              Absolutely! Test your knowledge and learn fun facts about biking
              by taking part in our interactive quizzes.
            </p>
          </li>
          <br></br>
          <li>
            <h6 className="font-semibold">
              6. Is ByCycle a casual community or more competitive?{" "}
            </h6>

            <p>
              {" "}
              ByCycle is all about fostering a casual and welcoming environment
              for cyclists of all levels to connect and share their passion for
              biking.
            </p>
          </li>
          <br></br>
          <li>
            <h6 className="font-semibold">
              7. How can I contribute to the ByCycle page?
            </h6>

            <p>
              Feel free to share your own biking experiences, trail
              recommendations, event announcements, or any other cycling-related
              content with the community.
            </p>
          </li>
          <br></br>
          <li>
            <h6 className="font-semibold">
              8. Do you offer tips and advice for beginner cyclists?{" "}
            </h6>

            <p>
              {" "}
              Yes, ByCycle provides helpful tips, resources, and support for
              those just starting out in the world of cycling.
            </p>
          </li>
          <br></br>
          <li>
            <h6 className="font-semibold">
              9. Can I connect with other cyclists in my area through ByCycle?
            </h6>

            <p>
              Absolutely! By engaging with our page, you can connect with
              like-minded individuals, join group rides, and even organize
              meetups.
            </p>
          </li>
          <br></br>
          <li>
            <h6 className="font-semibold">
              10. What sets ByCycle apart from other cycling communities?
            </h6>

            <p>
              {" "}
              ByCycle prides itself on its inclusive and friendly atmosphere,
              where cyclists of all backgrounds can come together to share their
              love for biking.
            </p>
          </li>
        </ol>
      </div>
    </div>
  );
}
